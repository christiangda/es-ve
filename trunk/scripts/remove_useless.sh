...
# espacios y tabuladores al final de cada linea y ordena en orden alfabético
# las cadenas pasadas como entrada y elimina los duplicados
#
# USO:  cat fichero_entrada | remove_useless.sh > fichero_salida
#
sed '/^\#/d;/./!d;s/^[ \t]*//;s/[ \t]*$//' | sort | uniq
